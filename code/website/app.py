...
def main():
    ...
    mag = st.slider('Moment Magnitude', 0.0, 9.0, 6.5)
    s0 = st.slider('S0', 67.4, 990.4, 262.9)
    sp0 = st.slider('sp0',216.7, 3816.1, 1020.0)
    spt = st.slider('Standard Penetration Test', 2.2, 65.5, 11.3)
    ag = st.slider('peak horizontal acceleration at ground surface', 0.0, 0.7, 0.2)
    tau_dinamic = st.slider('equivalent dynamic shear stress', 0.0, 0.5, 0.1)
    pfine = st.slider('Fines content', 0, 91, 26)
